...
function sumAll1(x: number, y: number): number {
    return x + y;
}
// 注意，输入多余的（或者少于要求的）参数，都是不被允许的：
/*function sumAll2(x: number, y: number): number {
    return x + y;
}
sumAll1(1, 2, 3);
...
let mySums = function (x: number, y: number): number {
    return x + y;
};
// 这是可以通过编译的，不过事实上，上面的代码只对等号右侧的匿名函数进行了类型定义，
// 而等号左边的 mySum，是通过赋值操作进行类型推论而推断出来的。如果需要我们手动给 mySum 添加类型，则应该是这样：
let mySums1: (x: number, y: number) => number = function (x: number, y: number): number {
    return x + y;
};
...
function pushy(array: any[], ...items: any[]) {
    items.forEach(function(item) {
        array.push(item);
    });
}
