...
class CacheManager {
  ...
  static setValue(key, value, expired) {
...
class Model {
  ...
  __propertys__() {
    ...
    // $.ajax代理
    this.ajax = ($ && $.ajax) ? $.ajax : function (options) {
      fetch(options.url, {
    ...
    this.userDataStore = {
      getValue: function () {
        return Utils.Storage.get('MEMBER_INFO')
      }
    }
    // 版本
  ...
  _getOptions(options) {
    ...
    return {
      ...
      beforeSend: function (request) {
        ...
        request.setRequestHeader('Access-Control-Allow-Methods', self.method)
      ...
      success: function (response) {
        ...
        if (!response.isSuccess) {
          if (response.code === 'LG301' || response.code === 'LG302' || response.code === 'LG303') {
            ...
            // $eventor.trigger('login_expired')
            $(document).trigger('login_expired', new Error(response.message))
            return
          }

          if (response.code === 'BCS15') {
            // Native.trigger('user_access_disable');
            $(document).trigger('user_access_disable', new Error(response.message))
            return
          }
        ...
        try {
          options.onSuccess && options.onSuccess(response.body, response)
        } catch (e) {
          console.error(e)
        }
      },
      error: function (res, status) {
        ...
        if (!('isSuccess' in res)) {
          if (!status) {
            result = {message: '操作失败！请稍候再试。\r\n出现这个提示可能是你的设备不能联网。', isSuccess: false, code: 'J01'}
